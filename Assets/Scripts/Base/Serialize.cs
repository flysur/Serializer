...
public class CSerialize
{
    ...
    public void SerializeStructMyArray<_Ty>(ref CMyArray<_Ty> aValue) where _Ty : ISerializable, new()
    {
        if (aValue == null)
            aValue = new CMyArray<_Ty>();
        int nLen = 0;
        if (IsLoading())
        {
            Read(ref nLen);
        ...
        {
            nLen = aValue != null ? aValue.size() : 0;
            Write(nLen);
            for (int i = 0; i < nLen; ++i)
            {
                _Ty value = aValue[i];
    ...
    public void SerializeMyArray<_Ty>(ref CMyArray<_Ty> aValue, SerializeArrayNode<_Ty> serializeFunc)// where _Ty : new()
    {
        if (aValue == null)
            aValue = new CMyArray<_Ty>();
